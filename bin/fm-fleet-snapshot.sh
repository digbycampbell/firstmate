...
# Growable JSON documents (backlog, tasks, secondmate summaries, and their
# aggregates) reach jq through --slurpfile from this private staging directory,
# never through --argjson: a single argv/envp string is capped at Linux's
# MAX_ARG_STRLEN (128KB), which a real backlog exceeds.
SNAPSHOT_TMP=$(umask 077; mktemp -d "${TMPDIR:-/tmp}/fm-fleet-snapshot.XXXXXX") \
  || { echo "fm-fleet-snapshot: cannot create staging directory" >&2; exit 1; }
trap 'rm -rf "$SNAPSHOT_TMP"' EXIT
# fm_run_timed TERMs a timed-out child summary; exit through the EXIT trap so
# the staging directory is still removed.
trap 'exit 129' HUP
trap 'exit 130' INT
trap 'exit 143' TERM

stage_doc() {  # <name> <json> - stage one JSON document; prints its file path
  printf '%s' "$2" > "$SNAPSHOT_TMP/$1.json" || return 1
  printf '%s' "$SNAPSHOT_TMP/$1.json"
}
...
task_json_lines() {
  ...
  local open_decisions_tsv open_decisions_json open_decisions_doc
  ...
  for meta in "$STATE"/*.meta; do
    ...
    open_decisions_doc=$(stage_doc task-open-decisions "$open_decisions_json") || return 1
    ...
    jq -n \
      ...
      --slurpfile open_decisions_docs "$open_decisions_doc" \
      --argjson pending_decision "$(bool_json "$pending_decision")" \
      --argjson blocked_event "$(bool_json "$blocked_event")" \
      --argjson report_present "$(bool_json "$report_present")" \
      '$open_decisions_docs[0] as $open_decisions
      | {
        id:$id,
        kind:$kind,
        harness:($harness // ""),
...
main_inventory_json() {  # <backlog-json> <tasks-json>
  local backlog_doc tasks_doc
  backlog_doc=$(stage_doc mi-backlog "$1") || return 1
  tasks_doc=$(stage_doc mi-tasks "$2") || return 1
  jq -n \
    --slurpfile backlog_docs "$backlog_doc" \
    --slurpfile tasks_docs "$tasks_doc" '
    $backlog_docs[0] as $backlog
    | $tasks_docs[0] as $tasks
    | ([ $backlog.records[]?
...
secondmate_home_summary_json() {  # <backlog-json> <tasks-json>
  local backlog_doc tasks_doc
  backlog_doc=$(stage_doc shs-backlog "$1") || return 1
  tasks_doc=$(stage_doc shs-tasks "$2") || return 1
  jq -n \
...
parent_evidence_reconciliation_json() {  # <summary-json> <activities-json> <decisions-json>
  local summary_doc activities_doc decisions_doc
  summary_doc=$(stage_doc recon-summary "$1") || return 1
  activities_doc=$(stage_doc recon-activities "$2") || return 1
  decisions_doc=$(stage_doc recon-decisions "$3") || return 1
  jq -n --slurpfile summary_docs "$summary_doc" \
    --slurpfile activities_docs "$activities_doc" \
    --slurpfile decisions_docs "$decisions_doc" '
    $summary_docs[0] as $summary
    | $activities_docs[0] as $activities
    | $decisions_docs[0] as $decisions
...
secondmate_current_json() {  # <parent-tasks-json>
  ...
  local seen_homes='' registry_doc tasks_doc records_doc
  registry=$(registry_secondmates_json) || return 1
  registry_doc=$(stage_doc sc-registry "$registry") || return 1
  tasks_doc=$(stage_doc sc-tasks "$tasks") || return 1
  union=$(jq -n --slurpfile registry_docs "$registry_doc" --slurpfile tasks_docs "$tasks_doc" '
    $registry_docs[0] as $registry
    | $tasks_docs[0] as $tasks
    |
  ...
  # Each record is appended to this staging file as its own JSON document;
  # the final assembly slurps them so the aggregate never rides argv.
  records_doc="$SNAPSHOT_TMP/sc-records.json"
  : > "$records_doc"
  ...
  while IFS= read -r row; do
    ...
    stage_doc sc-activity-scan "$activity_scan" >/dev/null || return 1
    stage_doc sc-activities "$activities" >/dev/null || return 1
    stage_doc sc-decisions "$decisions" >/dev/null || return 1
    ...
    if [ -z "$reason" ]; then
      ...
      stage_doc sc-summary "$summary" >/dev/null || return 1
      stage_doc sc-reconciliation "$reconciliation" >/dev/null || return 1
      record=$(jq -n \
        ...
        --argjson registered "$registered" --slurpfile summary_docs "$SNAPSHOT_TMP/sc-summary.json" --argjson summary_valid "$summary_valid" --slurpfile decisions_docs "$SNAPSHOT_TMP/sc-decisions.json" \
        --slurpfile activities_docs "$SNAPSHOT_TMP/sc-activities.json" --slurpfile activity_scan_docs "$SNAPSHOT_TMP/sc-activity-scan.json" \
        --slurpfile reconciliation_docs "$SNAPSHOT_TMP/sc-reconciliation.json" --argjson terminal "$terminal" --argjson contradiction "$contradiction" \
        --arg event_raw "$event_raw" --arg event_note "$event_note" --argjson event_age "$event_age" '
        $summary_docs[0] as $summary
        | $decisions_docs[0] as $decisions
        | $activities_docs[0] as $activities
        | $activity_scan_docs[0] as $activity_scan
        | $reconciliation_docs[0] as $reconciliation
      ...
      stage_doc sc-fallback-summary "$summary" >/dev/null || return 1
      record=$(jq -n \
        ...
        --argjson registered "$registered" --argjson event_age "$event_age" --slurpfile activities_docs "$SNAPSHOT_TMP/sc-activities.json" --slurpfile activity_scan_docs "$SNAPSHOT_TMP/sc-activity-scan.json" \
        --slurpfile decisions_docs "$SNAPSHOT_TMP/sc-decisions.json" --argjson terminal "$terminal" \
        --slurpfile summary_docs "$SNAPSHOT_TMP/sc-fallback-summary.json" --argjson summary_sampled "$summary_sampled" '
        $activities_docs[0] as $activities
        | $activity_scan_docs[0] as $activity_scan
        | $decisions_docs[0] as $decisions
        | $summary_docs[0] as $summary
...
EOF
  jq -n \
    --slurpfile registry_docs "$registry_doc" \
    --slurpfile records "$records_doc" \
    --argjson total_registered "$total_registered" \
    --argjson total "$total" \
    --argjson shown "$shown" \
    --argjson truncated "$truncated" \
    '{registry:$registry_docs[0],records:$records,total_registered:$total_registered,total:$total,shown:$shown,truncated:$truncated}'
}

secondmate_landed_from_current_json() {  # <secondmate-current-json>
  local current_doc
  current_doc=$(stage_doc landed-current "$1") || return 1
  jq -n --slurpfile current_docs "$current_doc" '
    $current_docs[0] as $current
...
stage_doc final-backlog "$BACKLOG_JSON" >/dev/null || exit 1
stage_doc final-tasks "$TASKS_JSON" >/dev/null || exit 1
stage_doc final-main-inventory "$MAIN_INVENTORY_JSON" >/dev/null || exit 1
stage_doc final-scout-reports "$SCOUT_REPORTS_JSON" >/dev/null || exit 1
stage_doc final-secondmate-current "$SECONDMATE_CURRENT_JSON" >/dev/null || exit 1
stage_doc final-secondmate-landed "$SECONDMATE_LANDED_JSON" >/dev/null || exit 1
...
jq -n \
  ...
  --arg config "$CONFIG" \
  --arg projects "$PROJECTS" \
  --slurpfile backlog_docs "$SNAPSHOT_TMP/final-backlog.json" \
  --slurpfile tasks_docs "$SNAPSHOT_TMP/final-tasks.json" \
  --slurpfile main_inventory_docs "$SNAPSHOT_TMP/final-main-inventory.json" \
  --slurpfile scout_reports_docs "$SNAPSHOT_TMP/final-scout-reports.json" \
  --slurpfile secondmate_current_docs "$SNAPSHOT_TMP/final-secondmate-current.json" \
  --slurpfile secondmate_landed_docs "$SNAPSHOT_TMP/final-secondmate-landed.json" \
  '$backlog_docs[0] as $backlog
   | $tasks_docs[0] as $tasks
   | $main_inventory_docs[0] as $main_inventory
   | $scout_reports_docs[0] as $scout_reports
   | $secondmate_current_docs[0] as $secondmate_current
   | $secondmate_landed_docs[0] as $secondmate_landed
