...
1. First action: create your branch: `git checkout -b fm/evidence-demo`
...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/tmp.zPHqJAdSOI/state/evidence-demo.status'`
...
6. If a decision belongs above the implementation worker (product choices, destructive actions, ask-user findings),
   ...
   When more than one decision or blocker can be open at once, name yours: write the key between the state and the colon, as `needs-decision [key=<slug>]: {summary}` or `blocked [key=<slug>]: {why}`.
...
# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/home/digby/.no-mistakes/worktrees/2c4342823de3/01M0BS6GVXBDFFWT7Z74MDHX31/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md` that lacks `## Maintaining this file`, add that short self-governance section from `/home/digby/.no-mistakes/worktrees/2c4342823de3/01M0BS6GVXBDFFWT7Z74MDHX31/bin/fm-ensure-agents-md.sh` in the same pass.
