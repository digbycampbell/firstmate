...
`bin/fm-test-sandbox-lib.sh` is the single owner of the suite's containment boundary: every test runs with the `FM_*` namespace cleared against an explicit allowlist, a private `TMPDIR`, and a shim that refuses the real `treehouse` binary.
