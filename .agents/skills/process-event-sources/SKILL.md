...
description: >-
  ...
  Use before arming a long-polling source firstmate owns, before registering a deterministic condition->action watch, and on any `procevent <adapter> <source-id> <sequence>` check wake.
...
Answer in the terminal and let the mirror carry your reply into Slack rather than hand-posting each turn's reply yourself; before the turn ends, record the thread you are answering with `bin/fm-slack-mirror.sh note-reply-target <channel> <thread-ts>` (or `none` for the channel top level) so the mirror files it into that exact thread even when captain messages interleave, which the newest-inbound guess gets wrong.
