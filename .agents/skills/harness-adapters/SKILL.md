...
description: >-
  Agent-only reference for firstmate harness operations, with verified facts for claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, and muse.
...
The firstmate PRIMARY's own `.claude/settings.json` registers three Stop hooks: `bin/fm-turnend-guard.sh --claude`, the Stop-owned auto-arm `bin/fm-claude-stop-autoarm.sh` (`asyncRewake: true`, `timeout: 28800`), and the Slack mirror `bin/fm-slack-mirror.sh stop`, which Grok registers separately in `.grok/hooks/fm-primary-slack-mirror.json` (see `docs/turnend-guard.md`), and exiting the guard with status 2 plus stderr reliably forces the model to continue.
