...
`bin/fm-slack-mirror.sh` owns that mirror and its header owns the whole contract: the Claude-only turn-end scope, the substantive-content and repeat suppression rules, how a deliberate post suppresses the mirror for that turn, how a reply is threaded back into the captain thread it answers, and its own state under `state/slack-captain/`.
