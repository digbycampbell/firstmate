...
`bin/fm-slack-mirror.sh` owns that mirror and its header owns the whole contract: the Claude-only turn-end scope, the substantive-content and repeat suppression rules, how a deliberate post suppresses the mirror for that turn, how firstmate records the thread it is answering with `note-reply-target` so the reply is routed back into that thread deterministically rather than by guessing the newest inbound, and its own state under `state/slack-captain/`.
