...
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  Grok registers a second, independent `Stop` hook in `.grok/hooks/fm-primary-slack-mirror.json` for the Slack mirror, the counterpart of Claude's third Stop entry and equally outside this guard.
...
`FM_SLACK_MIRROR_LIVE_E2E=1 tests/fm-slack-mirror-live-e2e.test.sh` is the opt-in guard that proves the same behavior against the installed harness and fails naming it and its version; [`verification/supervision.md`](verification/supervision.md#slack-mirror-turn-end-adapters) records the dated per-harness payload evidence.
