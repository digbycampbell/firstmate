...
`tests/fm-slack-mirror.test.sh` covers the Slack mirror over real Stop payloads and real transcripts: the substantive and configurable acknowledgement rules, the consecutive-repeat test, thread routing and binding expiry, the deterministic recorded reply target and its consume-once binding over the newest-inbound fallback, the adapter's reply-target record, deliberate-post suppression, every fail-open path, off-by-default scope, child-worktree inertness, and final-message selection.
