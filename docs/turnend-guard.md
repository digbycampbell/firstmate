...
- Claude registers three `Stop` hooks in `.claude/settings.json`, all anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`, `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`, and `bin/fm-slack-mirror.sh stop`, which mirrors the turn's final captain-facing message into the configured Slack captain channel.
  ...
  The mirror also reads the turn's own opening message from that transcript to thread a reply automatically into the Slack thread of the captain message that triggered the turn, with no manual step; `bin/fm-slack-mirror.sh` owns that resolution, and reading the opening message is the same Claude transcript surface, so auto-detect is Claude-only too and a future harness supplies its own trigger read against the same correlation store.
