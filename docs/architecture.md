...
Ship briefs also tell the crewmate to verify `pwd -P` and `git rev-parse --show-toplevel` before creating its branch (`issue-<n>` when the brief carries a linked issue, else `fm/<id>`), then stop with a blocked status if it landed in firstmate's permanent clone.
