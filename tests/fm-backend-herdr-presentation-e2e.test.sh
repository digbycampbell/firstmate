...
# Every task this fixture spawns stays live for the whole file: its worktree is
# only free for the pool to hand out again once its process ends, and a spawn
# onto a slot a live task still records is refused (fm-worktree-claim-lib.sh).
spawn_task() {  # <id> <home> <project>
  local id=$1 home=$2 project=$3
  FM_GATE_REFUSE_BYPASS=1 FM_SPAWN_NO_GUARD=1 FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/fm-spawn.sh" "$id" "$project" "sh -c 'sleep 3600'" --mode no-mistakes --yolo off --backend herdr
}
...
spawn_secondmate_task() {
  ...
  FM_GATE_REFUSE_BYPASS=1 FM_SPAWN_NO_GUARD=1 FM_HOME="$HOME_DIR" FM_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/fm-spawn.sh" "$id" "$home" "sh -c 'sleep 3600'" --secondmate --backend herdr
}
...
# The restart fixtures below stop the whole session, which frees every pool slot
# this project's live tasks are still recorded against, so they take their own
# project (and therefore their own pool) rather than colliding with those
# records (bin/fm-worktree-claim-lib.sh).
RESTART_PROJECT_DIR="$TMP_ROOT/restart-project"
