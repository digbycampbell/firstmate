...
# `shellcheck bin/*.sh bin/backends/*.sh bin/slack-mirror/**/*.sh tests/*.sh`,
# so PRs passed local
...
test_list_files_reports_the_shell_inventory() {
  ...
  expected=$(find bin bin/backends bin/slack-mirror bin/slack-mirror/adapters tests \
    -maxdepth 1 -type f -name '*.sh' -print | LC_ALL=C sort)
